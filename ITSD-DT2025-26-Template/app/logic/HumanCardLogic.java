...
public class HumanCardLogic {
        ...
        if ("Gloom Chaser".equals(cardName)) {
            ...
            for (int x = ux - 1; x <= ux + 1; x++) {
                for (int y = uy - 1; y <= uy + 1; y++) {
                    ...
                    if (target != null && target.getOwner() == 2 && target.getHealth() < target.getMaxHealth()) {
                        BasicCommands.playUnitAnimation(out, target, UnitAnimationType.death);
                        sleep(700);
                        BasicCommands.deleteUnit(out, target);
                        gameState.removeUnit(x, y);
    ...
    private static void applyBaseStats(Unit unit, String cardName) {
        ...
        if ("Bad Omen".equals(cardName)) { attack = 0; health = 1; }
        else if ("Gloom Chaser".equals(cardName)) { attack = 3; health = 1; }
        else if ("Rock Pulveriser".equals(cardName)) { attack = 1; health = 4; unit.setProvoke(true); }
        else if ("Shadow Watcher".equals(cardName)) { attack = 3; health = 2; }
        else if ("Nightsorrow Assassin".equals(cardName)) { attack = 4; health = 2; }
        else if ("Bloodmoon Priestess".equals(cardName)) { attack = 3; health = 3; }
        else if ("Shadowdancer".equals(cardName)) { attack = 5; health = 4; }
