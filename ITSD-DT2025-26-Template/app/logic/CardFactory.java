...
public class CardFactory {
    ...
    //    private static final int HUMAN_CARD_ID_START;
//    private static final int AI_CARD_ID_START; redundant now
    private static int cardIdCounter = 100;

    public static List<Card> buildHumanDeck() {
        List<Card> deck = new ArrayList<>();

        for (int copy = 0; copy < 2; copy++) {//2 copies of each card
        ...
        System.out.println("[CardFactory] Human Abyssian deck built: " + deck.size() + " cards.");
        return deck;
    }
    ...
    public static List<Card> createLyonarDeck() {
        ...
        Collections.shuffle(deck);
        System.out.println("[CardFactory] AI Lyonar deck built: " + deck.size() + " cards.");
        return deck;
    }
    ...
    public static String getUnitConfigByCardName(String cardName) {
        ...
        if (cardName.contains("Guardian") || cardName.contains("Ironcliff"))
            return "conf/gameconfs/units/ironcliff_guardian.json";
...
//      vvvv redundant, we use createLyonardDeck now instead
